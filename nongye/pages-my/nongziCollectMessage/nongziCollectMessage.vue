	...
	<view class="communityBlogFather">
		<scroll-view
			...
			<view class="blogPhotoFather">
				<image class="blogPhoto" :src="feedMessage.nongziCollectImage" mode="aspectFill" @click="previewImage"></image>
			...
			<view class="userProfile flex-between">
				<view class="blogTitleFather flex-start">
					<view class="blogTitle flex-center">{{feedMessage.nongziCollectName}}</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			previewImage() {
				...
				//目前这里不能浏览本地的图片
				uni.previewImage({
					urls: [_this.feedMessage.nongziCollectImage]
				})
			},
			clickAttention(){
				let _this = this
		...
		async onLoad(params) {
			...
			uni.getStorage({
				...
				success: function (res) {
					...
					uni.hideLoading()
					_this.splitContent = splitSentense(res.data.nongziCollectContent)
...
<style lang="stylus">
@import 'nongziCollectMessage.styl';
</style>
