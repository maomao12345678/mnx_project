	...
	<view class="communityBlogFather">
		<scroll-view
			...
			<view class="blogPhotoFather">
				<image class="blogPhoto" :src="medicineMessage.medicineImage" mode="aspectFill" @click="previewImage"></image>
			...
			<view class="userProfile flex-between">
				<view class="blogTitleFather flex-start">
					<view class="blogTitle flex-center">{{medicineMessage.medicineName}}</view>
				</view>
			...
			<view class="blogContentFather flex-center">
				<view class="blogContent" v-for="(content, index) in splitContent">
					<!-- {{medicineMessage.medicineContent}} -->
					{{content}}
				</view>
			</view>
		</scroll-view>
	...
	export default {
		...
		methods: {
			...
			previewImage() {
				...
				//目前这里不能浏览本地的图片
				uni.previewImage({
					urls: [_this.medicineMessage.medicineImage]
				})
			},
			clickAttention(){
				let _this = this
		...
		async onLoad(params) {
			...
			uni.getStorage({
				...
				success: function (res) {
					_this.medicineMessage = res.data
					uni.hideLoading()
					//看是否包含标点符号
					if(/[！。？]/.test(res.data.medicineContent)){
						_this.splitContent = splitSentense(res.data.medicineContent)
					}else{
						_this.splitContent.push(res.data.medicineContent)
					}
