	...
	<view class="communityBlogFather">
		<scroll-view
			...
			<view class="blogPhotoFather">
				<image class="blogPhoto" :src="fertilizerMessage.fertilizerImage" mode="aspectFill" @click="previewImage"></image>
			...
			<view class="userProfile flex-between">
				<view class="blogTitleFather flex-start">
					<view class="blogTitle flex-center">{{fertilizerMessage.fertilizerName}}</view>
				</view>
			...
			<view class="blogContentFather flex-center">
				<view class="blogContent" v-for="(content, index) in splitContent">
					<!-- {{fertilizerMessage.fertilizerContent}} -->
					{{content}}
				</view>
			</view>
		</scroll-view>
	...
	export default {
		...
		methods: {
			...
			previewImage() {
				...
				//目前这里不能浏览本地的图片
				uni.previewImage({
					urls: [_this.fertilizerMessage.fertilizerImage]
				})
			},
			clickAttention(){
				let _this = this
		...
		async onLoad(params) {
			...
			// 进入页面获取缓存数据
			uni.getStorage({
				key: 'fertilizerMessage',
				success: function (res) {
					_this.fertilizerMessage = res.data
					uni.hideLoading()
					//看是否包含标点符号
					if(/[！。？]/.test(res.data.fertilizerContent)){
						_this.splitContent = splitSentense(res.data.fertilizerContent)
					}else{
						_this.splitContent.push(res.data.fertilizerContent)
					}
			...
			apiRequest("isCollect", {
				...
				.then(res => {
					...
					// console.log(_this.feedMessage.feedId)
